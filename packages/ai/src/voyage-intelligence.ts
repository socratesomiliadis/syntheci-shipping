...
export class AiIntelligenceConfigurationError extends Error {
  constructor() {
    super("AI intelligence is not configured. Set GROQ_API_KEY to generate voyage intelligence.");
...
export async function generateVoyageIntelligence({
  ...
  const { object } = await generateObject({
    model: chatModel(),
    schema: aiVoyageIntelligenceOutputSchema,
...
export function validateVoyageIntelligenceOutput(
  ...
  if (!parsed.success) {
    return { runSummary: "AI output did not match the voyage intelligence schema.", jobs: [], findings: [], dropped: { jobs: 0, findings: 0 } };
  }
  ...
  const findings: ReconciliationFindingDraft[] = parsed.data.findings.flatMap((finding) => {
    const evidence = resolveFindingEvidence(finding.evidence, registry, evidenceByRef);
    if (evidence.length === 0) {
      droppedFindings += 1;
      return [];
    }
  ...
  return {
    ...
    dropped: { jobs: droppedJobs, findings: droppedFindings },
  };
}
...
function resolveFindingEvidence(
  ...
  const refs = evidence.flatMap((item) => [
    item.sourceId,
    `${item.sourceType}:${item.sourceId}`,
    item.label,
  ]);
  return resolveEvidenceRefs(refs, registry, evidenceByRef);
}
